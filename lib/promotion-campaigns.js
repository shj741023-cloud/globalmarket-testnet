...
function createCampaign(input, now = new Date()) {
  ...
  if (!sponsorName || !productId || !['advertising', 'sponsorship'].includes(type) || Number.isNaN(startAt.getTime()) || Number.isNaN(endAt.getTime()) || endAt <= startAt) {
    throw Object.assign(new Error('광고주, 대상 상품, 구분, 올바른 노출 기간이 필요합니다.'), { code: 'INVALID_PROMOTION_CAMPAIGN', status: 400 });
  }
  return { id: input.id, sponsorName, productId, type, startAt: startAt.toISOString(), endAt: endAt.toISOString(), note: String(input.note || '').trim().slice(0, 1000), status: 'scheduled', createdBy: input.adminId, createdAt: now.toISOString() };
}
