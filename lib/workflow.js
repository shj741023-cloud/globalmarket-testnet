...
function openDispute(trade, input, now = new Date()) {
  requireParcelTrade(trade);
  if (input.gasFeeNoticeAccepted !== true) {
    const error = new Error('Gas fee notice agreement is required before opening a dispute.');
    error.code = 'GAS_FEE_NOTICE_REQUIRED';
    throw error;
  }
  ...
  return {
    ...
    gasFeeNoticeAccepted: true,
    gasFeeNoticeAcceptedAt: now.toISOString(),
    gasFeeAtAcceptance: 0.01,
    status: 'received',
    settlementHold: true,
