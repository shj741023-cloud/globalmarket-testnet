...
const { sanitizeAuditEntry } = require('./audit-sanitize');
...
class Store {
  ...
  normalizeState(parsed) {
    ...
    this.state.refunds ||= [];
    this.state.gasDebts ||= [];
    this.state.gasCompensations ||= [];
    this.state.compensationPayouts ||= [];
    this.state.disputes ||= [];
    this.state.reports ||= [];
    this.state.suggestions ||= [];
    for (const suggestion of this.state.suggestions) {
      suggestion.category ||= 'suggestion';
      suggestion.title ||= '기존 건의사항';
    }
    this.state.notifications ||= [];
    this.state.announcements ||= [];
    this.state.announcementReads ||= [];
    this.state.auditLogs ||= [];
    const auditBefore = JSON.stringify(this.state.auditLogs);
    this.state.auditLogs = this.state.auditLogs.map(sanitizeAuditEntry);
    this.auditLogsSanitized ||= auditBefore !== JSON.stringify(this.state.auditLogs);
    this.state.reviews ||= [];
    this.state.trustProfiles ||= [];
    this.state.trustEvents ||= [];
    this.state.favorites ||= [];
    this.state.promotionCampaigns ||= [];
    this.state.events ||= [];
  }
  ...
  async initialize() {
    ...
    if (result.rows[0]) {
      ...
      if (this.auditLogsSanitized) {
        await this.pool.query('UPDATE global_market_testnet_state SET state = $2::jsonb, updated_at = NOW() WHERE id = $1', ['primary', JSON.stringify(this.state)]);
        this.auditLogsSanitized = false;
      }
    } else {
      await this.pool.query(
