...
function normalizeImageData(value) {
  ...
  if (imageData.length > 450_000) {
    throw Object.assign(new Error('상품 사진은 압축 후 330KB 이하여야 합니다.'), { code: 'PRODUCT_IMAGE_TOO_LARGE' });
  }
  return imageData;
}
...
function validateProductInput(input) {
  ...
  return {
    value: { title, description, price, categoryId, methods, region: normalizeText(input.region || '미지정'), imageData: normalizeImageData(input.imageData) },
...
function updateOwnedProduct(product, sellerId, input) {
  ...
  const checked = validateProductInput({
    ...
    imageData: input.imageData ?? product.imageData
  });
...
module.exports = {
  CATEGORIES, REVIEW_KEYWORDS, normalizeImageData, validateProductInput, searchProducts,
