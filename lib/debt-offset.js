...
function offsetDebts(debts,userId,available,now=new Date()){
 ...
 const eligible=debts.filter(d=>d.userId===userId&&d.status==='confirmed_unpaid'&&new Date(d.restrictionStartsAt)<=now&&d.outstandingAmount>0).sort((a,b)=>new Date(a.createdAt)-new Date(b.createdAt));
