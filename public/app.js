...
function bindTradeActions(detail) {
  ...
  $('actionChecklistRefund')?.addEventListener('click', async () => { if (!confirm('전액환불 가스비 안내\n\n• 최초 결제 가스비 0.01 Pi: 반환 불가\n• 구매자 환불 송금 가스비 0.01 Pi: 추가 발생 가능\n• 예상 전체 가스비 0.02 Pi\n• 실제 부담 주체는 환불정책 판정 필요\n\nTestnet 모의환불을 계속할까요?')) return; try { await api(`/api/v1/testnet/checklist-trades/${trade.id}/full-refund`, { method: 'POST' }); await refresh(); } catch (error) { alert(error.message); } });
  $('actionChecklistPartialRefund')?.addEventListener('click', async () => { if (!confirm('부분환불 가스비 안내\n\n• 최초 결제 가스비 0.01 Pi: 반환 불가\n• 구매자 환불 송금 가스비 0.01 Pi\n• 판매자 정산 송금 가스비 0.01 Pi\n• 예상 전체 가스비 0.03 Pi\n• 실제 부담 주체는 환불정책 판정 필요\n\nTestnet 절반 부분환불을 계속할까요?')) return; try { await api(`/api/v1/testnet/checklist-trades/${trade.id}/partial-refund`, { method: 'POST' }); await refresh(); } catch (error) { alert(error.message); } });
