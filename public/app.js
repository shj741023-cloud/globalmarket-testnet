...
async function chooseTrade(productId) {
  ...
  $('tradeNotice').textContent = type === 'direct'
    ? '직거래는 플랫폼 안전거래가 아닙니다. 플랫폼은 결제·보관·정산·환불을 제공하지 않습니다.'
...
function bindTradeActions(detail) {
  ...
  $('actionDirectPlan')?.addEventListener('click', async () => { if (!confirm('직거래는 플랫폼 안전거래가 없으며 모든 확인과 결제는 당사자 책임입니다. 동의합니까?')) return; const scheduledAt = prompt('약속 일시(예: 2026-08-20 15:00)'); const place = prompt('약속 장소'); const paymentMethod = prompt('당사자 간 결제방법'); if (!scheduledAt || !place || !paymentMethod) return; try { await api(`/api/v1/trades/${trade.id}/direct`, { method: 'POST', body: JSON.stringify({ noticeAccepted: true, scheduledAt, place, paymentMethod }) }); await refresh(); } catch (error) { alert(error.message); } });
