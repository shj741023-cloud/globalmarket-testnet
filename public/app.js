...
async function openTradeDetail(tradeId) {
  ...
  const refundSection = detail.refund ? `<div class="detail-grid"><p><small>Testnet 환불상태</small><strong>${detail.refund.type === 'partial' ? '부분 모의환불 완료' : '전액 모의환불 완료'}</strong></p><p><small>구매자 모의환불액</small><strong>${escapeHtml(detail.refund.totalBuyerRefund)} Test-Pi</strong></p>${detail.refund.type === 'partial' ? `<p><small>판매자에게 남은 금액</small><strong>${escapeHtml(detail.refund.retainedAmount)} Test-Pi</strong></p>` : ''}</div>` : '';
  ...
  if (trade.purpose === 'pi_checklist' && trade.myRole === 'buyer' && trade.status === 'disputed' && !detail.refund) actions.push(['actionChecklistRefund', 'Testnet 전액 환불', 'primary']);
  if (trade.purpose === 'pi_checklist' && trade.myRole === 'buyer' && trade.status === 'disputed' && !detail.refund) actions.push(['actionChecklistPartialRefund', 'Testnet 절반 부분환불', 'secondary']);
...
function bindTradeActions(detail) {
  ...
  $('actionChecklistSettle')?.addEventListener('click', async () => { if (!confirm('Testnet 모의정산을 완료하고 거래를 최종 완료할까요? 실제 Pi는 이동하지 않습니다.')) return; try { await api(`/api/v1/testnet/checklist-trades/${trade.id}/settlement`, { method: 'POST' }); await refresh(); } catch (error) { alert(error.message); } });
  $('actionChecklistRefund')?.addEventListener('click', async () => { if (!confirm('Testnet 전액 모의환불을 완료할까요? 실제 Test-Pi는 이동하지 않습니다.')) return; try { await api(`/api/v1/testnet/checklist-trades/${trade.id}/full-refund`, { method: 'POST' }); await refresh(); } catch (error) { alert(error.message); } });
  $('actionChecklistPartialRefund')?.addEventListener('click', async () => { if (!confirm('0.005 Test-Pi는 판매자 몫으로 남기고 나머지를 부분 모의환불할까요? 실제 Test-Pi는 이동하지 않습니다.')) return; try { await api(`/api/v1/testnet/checklist-trades/${trade.id}/partial-refund`, { method: 'POST' }); await refresh(); } catch (error) { alert(error.message); } });
